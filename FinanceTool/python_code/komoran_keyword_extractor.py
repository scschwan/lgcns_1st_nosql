...
class KomoranKeywordExtractor:
    def __init__(self):
        ...
        try:
            ...
            #print("Komoran 초기화 완료")
        except:
            print("Komoran 초기화 실패")
            self.komoran = None
...
def extract_keywords(text, logger, separators, min_count=1, max_length=10):
    ...
    try:
        ...
        # Komoran 분석기 초기화 (싱글톤 패턴)
        global _extractor
        if '_extractor' not in globals():
            _extractor = KomoranKeywordExtractor()
            
        if not _extractor.komoran:
            logger.info("Komoran 초기화 실패")  # 로깅 추가
            return words
            
        # Komoran으로 형태소 분석
        try:
            morphs = _extractor.komoran.morphs(text)
