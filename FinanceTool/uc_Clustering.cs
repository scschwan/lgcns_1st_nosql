    ...
    public partial class uc_Clustering : UserControl
    {
        ...
        private async Task PerformSearch()
        {
            ...
            if (!"".Equals(target_keyword))
            {
                ...
                // 새로운 검색 결과 표시
                var searchResult = new SearchResult { Data = filteredData, TotalCount = filteredData.Rows.Count };
                await _clusteringManager.DisplaySearchResultAsync(searchResult);
        ...
        private async Task PerformSearchWithProgress(ProcessProgressForm progressForm)
        {
            ...
            //await PerformSearch();
            // PerformSearch 내용을 SearchCriteria로 변환하여 호출
        ...
        private async void ClearMergeClusterTable()
        {
            /*
            ...
            change_row_count();
            */
            var emptyCriteria = new SearchCriteria { Keywords = new List<string>() };
            await _clusteringManager.SearchAsync(emptyCriteria);
            change_row_count();
        }
...
private void AddRowToGrid(DataGridView dgv, DataRow row, DataTable dt)
{
    ...
    UltraSpeedMergedData mergedData, int extremeParallelism)
        ...
        /// <summary>
        /// 병합 해제 후 검색 엔진 갱신 (공통 사용)
        /// </summary>
        private async Task RefreshSearchEngineAfterUnmerge()
        {
            try
            {
                // 1. mergeClusterDataTable 업데이트
                mergeClusterDataTable = await EnrichWithRawTableDataAsync(DataHandler.finalClusteringData);

                // 2. ClusteringManager 새로고침  
                if (_clusteringManager != null)
                {
                    await _clusteringManager.RefreshDataAsync(mergeClusterDataTable);
                }
            ...
            {
                Debug.WriteLine($"검색 엔진 갱신 오류: {ex.Message}");
            }
        }
