...
namespace FinanceTool
{
    public partial class uc_preprocessing : UserControl
    {
        ...
        public async Task initUI()
        {
            ...
            originKeywordDataTable = await DataHandler_preprocessing.CreateDataTableFromColumnNamesAsync(DataHandler.processTable, selectedColumnNames);
            ...
            // 찾은 인덱스로 데이터 테이블 생성
            DataHandler.moneyDataTable = DataHandler_preprocessing.ExtractColumnToNewTable(DataHandler.processTable, moneyColumnIndex);
            DataHandler.lowLevelData = DataHandler_preprocessing.ExtractColumnToNewTable(DataHandler.processTable, targetColumnIndex);

            //modifiedDataTable = await DataHandler.CreateDataTableFromColumnsAsync(DataHandler.processTable, DataHandler.levelList);


            
            modifiedDataTable = await DataHandler_preprocessing.CreateDataTableFromColumnNamesAsync(DataHandler.processTable, selectedColumnNames);
            ...
            // 데이터그리드뷰 간 선택 동기화
            DataHandler_preprocessing.SyncDataGridViewSelections(dataGridView_target, dataGridView_applied);
        ...
        private async void btn_apply_Click(object sender, EventArgs e)
        {
            ...
            modifiedDataTable = DataHandler_preprocessing.ReplaceSeparatorInColumn(modifiedDataTable, modifiedDataTable.Columns.Count -1, "_", "separate");
            ...
            dataGridView_applied.DataSource = DataHandler_preprocessing.CombineDataTables(modifiedDataTable);
        ...
        private async void remove_apply_btn_Click(object sender, EventArgs e)
        {
            ...
            modifiedDataTable = DataHandler_preprocessing.ReplaceSeparatorInColumn(modifiedDataTable, modifiedDataTable.Columns.Count - 1, "", "remove");
            ...
            try
            {
                ...
                using (var progressForm = new ProcessProgressForm())
                {
                    ...
                    modifiedDataTable = await Task.Run(() => {
                        return DataHandler_preprocessing.ReplaceSeparatorInColumn(
                            modifiedDataTable,
                    ...
                    modifiedDataTable = await Task.Run(() => {
                        ...
                        // 짧은 문자열 전처리
                        var processedTable = DataHandler_preprocessing.ProcessShortStringsToNull(modifiedDataTable);

                        // 밑줄 문자 전처리
                        return DataHandler_preprocessing.ProcessUnderscoresInAllColumn(processedTable);
                    });
                    await progressForm.UpdateProgressHandler(70, "4/5 단계: 구분자 기반 분할 시작...");

                    // 6. 구분자 기반 추출 - 병렬 처리
                    modifiedDataTable = await Task.Run(() => {
                        return DataHandler_preprocessing.SplitColumnBySeparator(modifiedDataTable, "_");
                    });
        ...
        private async void keyword_model_split_Click(object sender, EventArgs e)
        {
            try
            {
                ...
                modifiedDataTable = await DataHandler_preprocessing.SplitColumnByModel(modifiedDataTable, ai_limit_cnt);
